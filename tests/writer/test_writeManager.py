...
class TestWriter:
    def test_write(self, writer, nonDefWriter):
        ...
        Path.unlink(Path(writer.outputDirectory + "/" + writer.fileName + ".txt").resolve())
        Path.rmdir(Path(writer.outputDirectory).resolve())

        nonDefWriter.write()
        assert Path(nonDefWriter.outputDirectory).is_dir()
        assert Path(nonDefWriter.outputDirectory + "/" + nonDefWriter.fileName + ".txt").is_file()
        Path.unlink(Path(nonDefWriter.outputDirectory + "/" + nonDefWriter.fileName + ".txt").resolve())
